  ...
  mlir::ModuleOp mlirGen(ModuleAST &moduleAST) {
    ...
    for (auto &record : moduleAST) {
      if (FunctionAST *funcAST = llvm::dyn_cast<FunctionAST>(record.get())) {
        mlir::mytoy::FuncOp func = mlirGen(*funcAST);
        if (!func)
          return nullptr;
...
private:
  ...
  llvm::StringMap<mlir::mytoy::FuncOp> functionMap;
  ...
  mlir::Value mlirGen(CallExprAST &call) {
    ...
    mlir::mytoy::FuncOp calledFunc = calledFuncIt->second;
  ...
  mlir::Value mlirGen(ExprAST &expr) {
    ...
    case mytoy::ExprAST::Expr_StructLiteral:
